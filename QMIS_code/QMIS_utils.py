"""
File containing the class of the quantum analog computing MIS finder utilities functions. They are all used in the Quantum_MIS in the Quantum_MIS.py file.
"""
...
def scale_coordinates(
    ...
    """
    Function that scale the coordinates of a netwrokx graph that was layed-out to transform them into coordinates
...
def fusion_counts(counts: List[dict], positions: List[int]) -> dict:
    """
    Fusions the a list of dictionary counts into one with the combined bitstrings. The combined result bitstrings will be
    generated by their given positions.

    Parameters:
    - counts List[dict]): The list of counts resulted by the runs of the QMIS on subgraphs.
    - positions (List[int]): The subgraph index containing the ith node.

    Returns:
    dict: The combined counts dictionary of the subgraphs.
    """
    total_counts = {}
    ...
    for combination in bitstring_combinations:
        ...
        combined_value = 0
        # place each bit in its given position with the combined value
